...
class AlipayTradeService {
	...
	function __construct($alipay_config){
		...
		$this->charset = "UTF-8";
		$this->MaxQueryRetry = 10;
		$this->QueryDuration = 3;
	...
	public function qrPay($req) {
		...
		// 首先调用支付api
		$response = $this->aopclientRequestExecute (0, $request, NULL ,$req->getAppAuthToken() );
	 ...
	 */
	private function aopclientRequestExecute($num,$request, $token = NULL, $appAuthToken = NULL) {
		...
		// 开启页面信息输出
		$aop->debugInfo=true;
		$result = $aop->execute($request,$token,$appAuthToken,$num);
