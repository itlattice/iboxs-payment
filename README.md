...
* 本项目目前已支持支付宝/微信的部分常用接口（扫码/网页/手机/公众号支付及回调验签），其他接口陆续更新中。
...
##### 参数
* 网页支付（AlipayWeb）与扫码支付（AlipayCode）中传入的$orderInfo为数组类型，含有参数为：
``` 
$orderInfo=array(
...
$result=Notify::alipayNotify($alipayconfig);  //调用支付宝异步验签 //返回布尔型，真为验证通过，可以进行下一步，假为验证失败
if($result==true){
    //进行订单操作(这里不用输出success，接口内已经输出了)
} else{
    echo 'fail';
}
```
...
$result=Notify::WxPayNotify($wxpayconfig);  //调用微信回调验签 //返回布尔型，真为验证通过，可以进行下一步，假为验证失败
if($result==true){
