<?php
/**
 * 这里是微信支付商户订单号查询订单demo
 * 文档地址：https://pay.weixin.qq.com/doc/v3/merchant/4012791859
 */
...
$out_trade_no="2021101247845559";   //商户端订单号
try{
    $config=require("./config/config.php");  //可将config_example.php复制为config.php，填入自己的参数，并在这里引入
    $payment= Payment::setConfig($config);//laravel/thinkphp框架下按文档说明把配置文件放入config文件夹下后，无需调用本函数，可直接使用Payment::wechatOutTradeQuery()
    $payment=$payment->wechatOutTradeQuery($out_trade_no);
