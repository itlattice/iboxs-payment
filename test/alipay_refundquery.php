<?php
/**
 * 这里是支付宝统一收单交易退款查询接口demo
 * 文档地址：https://opendocs.alipay.com/open/c1cb8815_alipay.trade.fastpay.refund.query
 */
...
$out_request_no='2021101247845559';   //退款请求号(请求退款接口时，传入的退款请求号，如果在退款请求时未传入，则该值为创建交易时的商户订单号。)
...
try{
    ...
    $payment= Payment::setConfig($config);  //laravel/thinkphp框架下按文档说明把配置文件放入config文件夹下后，无需调用本函数，可直接使用Payment::alipayRefundQuery()
    $payment=$payment->alipayRefundQuery($out_request_no,$out_trade_no,$trade_no);
