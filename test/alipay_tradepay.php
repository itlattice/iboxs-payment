 ...
 * 文档地址：https://opendocs.alipay.com/open/1f1fe18c_alipay.trade.pay
 */
...
$amount=1;   //订单金额，单位元
$subject="订单测试";  //订单标题
$authCode="123456789"; //付款码
try{
    $config=require("./config/config.php");  //可将config_example.php复制为config.php，填入自己的参数，并在这里引入
    $payment= Payment::setConfig($config);  //laravel/thinkphp框架下按文档说明把配置文件放入config文件夹下后，无需调用本函数，可直接使用Payment::alipayTradePay()
    $payment=$payment->alipayTradePay($no,$amount,$subject,$authCode);
